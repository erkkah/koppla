...
async function render(schematic, skin, options = {
    ...
    return renderSVG(laidOut, font, skin, !!options.drawBoxes);
}
exports.render = render;
function labelsInTree(node) {
    var _a, _b, _c;
    const labels = [
        ...((_a = node.labels) !== null && _a !== void 0 ? _a : []).map((label) => label),
        ...((_b = node.ports) !== null && _b !== void 0 ? _b : []).flatMap((port) => { var _a; return (_a = port.labels) !== null && _a !== void 0 ? _a : []; }),
        ...((_c = node.children) !== null && _c !== void 0 ? _c : []).flatMap((child) => labelsInTree(child)),
    ];
    return labels;
}
...
function renderSVG(layout, font, skin, drawBoxes) {
...
function minify(code) {
    const mini = code.replace(/^\s+/gm, "").replace(/[{]([^}]+)[}]/gm, (_all, style) => {
        var _a;
        const oneLine = ((_a = style === null || style === void 0 ? void 0 : style.split("\n")) !== null && _a !== void 0 ? _a : []).join("");
        return `{${oneLine}}`;
    });
    return mini;
}
