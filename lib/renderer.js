...
async function render(schematic, symbols, skin, options = {
    optimize: true,
    fontSize: 20,
}) {
    ...
    const layoutOptions = Object.assign({ "elk.algorithm": "layered", "elk.direction": "DOWN", "elk.edgeRouting": "ORTHOGONAL", "elk.spacing.labelLabel": 3, 
        //"elk.layered.layering.strategy": "NETWORK_SIMPLEX",
        "elk.layered.layering.strategy": "LONGEST_PATH", "elk.layered.nodePlacement.strategy": "BRANDES_KOEPF", 
        //"elk.layered.nodePlacement.strategy": "NETWORK_SIMPLEX",
        "elk.layered.compaction.postCompaction.strategy": "LEFT", "elk.edge.thickness": 3.5 }, layoutOptionsFromSettings);
    /*
...
function renderSVG(layout, font, skin) {
    ...
    const svgLabels = layout.children.flatMap((node) => {
        ...
        return labels.map((label) => {
            ...
            return (`<text x="${x}" y="${y}" alignment-baseline="hanging">${label.text}</text>` +
                (DEBUG
